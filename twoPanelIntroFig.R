...
units <- fullset[1,]
fullset <- fullset[-1,]
tempStart <- as.Date("2017-12-01")
tempEnd  <- as.Date("2020-03-28")
...
datSeq <- seq(as.Date("2018-03-01"), as.Date("2020-02-28"), by = "month")

startDate <- as.Date("2017-12-01")
endDate  <- as.Date("2020-03-28")
pchS <- c(1, 2, 0, 8, 6)
...
plot(subTotals$Date, subTotals$DailyPar, xaxt = "n", ylab = "", yaxt = "n", 
     ...
     type = "b", xlim = c(as.Date("2018-01-27"), as.Date("2019-12-06")))
box(lwd = 2.5)
...
axis(1, cex.axis=2.5, labels = NA, 
     at = as.numeric(seq(as.Date("2018-01-01"), as.Date("2019-12-31"), 
...
tiff("FigBinExtras/SurfLightandTemp_Intro.tiff", width = 7, height = 6, 
     pointsize = 12, units = "in", res = 400)

par(mar = c(1, 3, 1, 0), xpd = NA)
plot(0:2, 0:2, type = "n", xaxt = "n", yaxt = "n", xlab = "", ylab = "", bty = "n")
rasterImage(readPNG(source="FigBinExtras/surfaceLightMLD.png"), -0.16, 1.3, 1.9, 2.1)
rasterImage(readPNG(source="FigBinExtras/tempProfileSub.png"), -0.15, -0.025, 1.9, 1.3)
mtext("Depth (m)", 2, line = 1.4, adj = 0.33)
mtext(expression("PAR (mol photons m"^{"-2"}~"d"^{"-1"}~")"), 2, line = 1.3, 
      adj = 0.98)
...
depthAdj <- seq(-3.8, 17.5, length.out = length(depthSeq))
mtext(depthSeq, 2, line = 0.22, padj = depthAdj, las = 1, cexSmall)
datSeq0 <- format(datSeq[seq(1, length(datSeq), 3)], "%b")
datAdj <- seq(0.05, 0.82, length.out = length(datSeq0))
mtext(datSeq0, 1, line = -1.85, adj = datAdj, las = 1, cexSmall)
mtext("Date", 1, line = -0.2,adj = 0.44)
tempSeq <- seq(0, 20, by = 5)
tempAdj <- seq(20.1, -3.7, length.out = length(tempSeq))
mtext(tempSeq, 2, line = -28.5, padj = tempAdj, las = 1, cexSmall, adj = 0)
